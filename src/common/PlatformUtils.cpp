...
long openFileForRead(const std::string &path) {
  ...
  HANDLE hFile = CreateFileA(path.c_str(), GENERIC_READ, FILE_SHARE_READ, &sa,
...
long openFileForWrite(const std::string &path, bool append) {
  ...
  HANDLE hFile =
      CreateFileA(path.c_str(), append ? FILE_APPEND_DATA : GENERIC_WRITE,
                  FILE_SHARE_READ, &sa, creation, FILE_ATTRIBUTE_NORMAL, NULL);
  if (hFile == INVALID_HANDLE_VALUE)
    return -1;
  if (append) {
...
std::string toNativePath(const std::string &path) {
#ifdef _WIN32
  // Convert forward slashes to backslashes for Windows API calls
  std::string result = path;
  for (char &c : result) {
    if (c == '/') {
      c = '\\';
  ...
  return result;
#else
  // Unix already uses forward slashes
  return path;
#endif
}

std::string normalizeLineEndings(const std::string &text) {
  // Strip \r from CRLF to get LF only
  std::string result;
